...
import{WORLD,TOWN,REGIONS,SERVICES,FIELD_PACKS,RIFT_ENTRY,inTown,regionAt,worldObjects}from'./world.mjs';
...
export class Game{
  constructor(save=freshSave(),seed=Date.now()){
    this.save=save;this.rng=seeded(seed);this.players=[];this.enemies=[];this.shots=[];this.zones=[];this.fx=[];this.texts=[];this.drops=[];this.events=[];this.chests=[];this.rooms=ROOMS.map((r,i)=>({...r,id:i,state:'locked',discovered:i===0}));this.state='title';this.paused=false;this.time=0;this.depth=1;this.room=0;this.kills=0;this.runGold=0;this.runLegends=0;this.portal=null;this.shake=0;this.dropTurn=0;this.completed=false;this.titleClock=0;this.summons=[];this.pending=[];this.hitStop=0;this.flash=0;this.townChannel=null;this.townReturn=null;this.scene='town';this.riftInstance=null;this.obstacles=this.makeObstacles();
  }
  makeObstacles(){return worldObjects(this.scene).filter(o=>o.solid);}
  ...
  begin(classes=['knight'],depth=1){
    this.scene='town';this.riftInstance=null;this.obstacles=this.makeObstacles();this.bossKey=this.save.settings.boss||'ember';this.rift={phase:'ready',progress:0,goal:100,elapsed:0,limit:900,timed:false};this.summons=[];this.pending=[];this.hitStop=0;this.flash=0;this.townChannel=null;this.townReturn=null;
    this.save.lastClasses=classes;this.depth=clamp(Math.floor(depth),1,this.save.bestDepth);this.players=classes.map((cls,index)=>{const key=`${index}:${cls}`;const profile=this.save.profiles[key]??freshProfile(cls);normalizeWeapons(profile);this.save.profiles[key]=profile;profile.runes??=[0,0,0];profile.skillChoices??=[0,1,2];const s=stats(profile);return{id:index,cls,scene:this.scene,profile,stats:s,x:index*1.5-.75,z:4.5,vx:0,vz:0,face:{x:0,z:-1},hp:s.hp,mana:100,cd:[0,0,0],attackCd:0,potionCd:0,frostCd:0,chargeResource:0,focus:0,heat:0,overheated:0,overdrive:0,bear:0,ward:0,battleBuff:0,traitHits:0,lastSkill:-1,legendCd:0,actionPose:null,invuln:1,anim:0,animType:'idle',walk:0,down:false,revive:0,combo:0,mark:null,hit:0};});
    this.enemies=[];this.shots=[];this.zones=[];this.fx=[];this.texts=[];this.drops=[];this.events=[];this.chests=[];this.rooms=ROOMS.map((r,i)=>({...r,id:i,state:i===0?'clear':i===4?'locked':'active',discovered:i===0}));this.room=0;this.time=0;this.kills=0;this.runGold=0;this.runLegends=0;this.dropTurn=0;this.portal=null;this.completed=false;this.paused=false;this.state='playing';this.room=0;this.toast('새벽불 마을 · 장비를 정비하고 균열에 도전하세요.','normal','C 마을 정비 · G 지도 · B 마을 귀환');this.event('save');
  }
  setScene(scene){this.scene=scene;for(const p of this.players){p.scene=scene;p.mark=null;p.vx=0;p.vz=0;}this.obstacles=this.makeObstacles();this.shots=[];this.zones=[];this.fx=[];this.texts=[];this.summons=[];this.pending=[];this.shake=0;this.hitStop=0;this.flash=0;}
  enterRift(){if(this.state!=='playing'||this.rift?.phase!=='ready'||!this.partyInTown())return false;this.setScene('rift');for(const p of this.players){p.x=RIFT_ENTRY.x+p.id*2-1;p.z=RIFT_ENTRY.z;p.invuln=2;}for(let i=1;i<4;i++)this.spawnRoom(i,true);this.room=regionAt(this.players[0]);this.startRift();return true;}
  startRift(){if(this.scene!=='rift'||this.rift?.phase!=='ready')return;this.rift.phase='hunting';this.toast('잿불 균열 입장 · 적을 처치해 진행도를 채우세요.','normal','정예는 더 많은 진행도 · 15분 안에 완료하면 추가 보상');}
  leaveRift(){if(this.scene!=='rift')return false;this.riftInstance={enemies:this.enemies,drops:this.drops,chests:this.chests,portal:this.portal};this.enemies=[];this.drops=[];this.chests=[];this.portal=null;this.setScene('town');for(const p of this.players){p.x=p.id*2-1;p.z=4.5;p.invuln=2;}this.room=0;this.restTown();return true;}
  ...
  spawnRoom(index,quiet=false){
    const r=this.rooms[index];if(index===0||this.scene!=='rift')return;r.state='active';r.discovered=!quiet;if(!quiet)this.room=index;
  ...
  spawnEnemy(type,x,z,room){
    if(this.scene!=='rift')return null;
  ...
  allowed(){return true;}
  move(entity,dx,dz,radius=.35,gate=true){const enemy=!gate&&Number.isInteger(entity.room),fits=(x,z)=>this.scene==='rift'||!enemy||!inTown({x,z});const steps=Math.max(1,Math.ceil(Math.max(Math.abs(dx),Math.abs(dz))/.25));for(let i=0;i<steps;i++){const sx=dx/steps,sz=dz/steps;if(fits(entity.x+sx,entity.z)&&this.walkable(entity.x+sx,entity.z,radius))entity.x+=sx;if(fits(entity.x,entity.z+sz)&&this.walkable(entity.x,entity.z+sz,radius))entity.z+=sz;}}
  inTown(p){return inTown(p);}
  partyInTown(){return this.players.length>0&&this.players.every(p=>inTown(p));}
  requestTown(){if(this.scene==='town'&&this.state==='playing'){for(const p of this.players){p.x=p.id*2-1;p.z=4.5;p.mark=null;}this.room=0;this.restTown();return true;}if(this.state!=='playing'||this.scene!=='rift'||this.players.some(p=>p.down)||this.townChannel)return false;if(this.players.some(p=>this.enemies.some(e=>!e.dead&&distance(p,e)<12))){this.toast('적에게서 조금 더 벗어난 뒤 귀환할 수 있습니다.','error');return false;}for(const p of this.players)p.mark=null;this.townChannel={remaining:2.5,origins:this.players.map(p=>({x:p.x,z:p.z}))};this.toast('마을 귀환 · 2.5초간 가만히 계세요.');return true;}
  returnToField(){if(!this.partyInTown()||!this.townReturn||!this.riftInstance)return false;const points=this.townReturn;Object.assign(this,this.riftInstance);this.riftInstance=null;this.setScene('rift');for(const p of this.players){const q=points[p.id];p.x=q.x;p.z=q.z;p.mark=null;p.invuln=2;}this.townReturn=null;this.summons=[];this.pending=[];this.event('sound',{name:'spell'});return true;}
  ...
  basic(p,input){
    if(p.down||this.scene!=='rift'||p.attackCd>0||p.overheated>0)return false;
  ...
  summon(p,type,count,point,damage,opts={}){const existing=this.summons.filter(s=>s.owner===p&&s.type===type);const cap=opts.cap||(type==='turret'?2:4);for(let i=0;i<count;i++){if(existing.length>=cap){const old=existing.shift();this.summons=this.summons.filter(s=>s!==old);}const a=i*2.4+({archer:1.2,golem:3.5,wolf:.6}[type]||0),formation=type==='archer'?3.5:type==='golem'?2.5:2,s={type,owner:p,followX:Math.cos(a)*formation,followZ:Math.sin(a)*formation,x:point.x+Math.cos(a)*formation,z:point.z+Math.sin(a)*formation,life:opts.life||12,cd:.2+i*.15,damage,range:opts.range||10,interval:opts.interval||1,angle:0};if(!this.walkable(s.x,s.z,.4)){s.x=p.x;s.z=p.z;}this.summons.push(s);existing.push(s);this.effect('summon',s.x,s.z,type==='turret'?'#ffce86':'#83ebd0',{r:1.5,life:.8});}}
  skill(p,index,input){
    if(p.down||this.scene!=='rift'||!Number.isInteger(index)||index<0||index>2||p.cd[index]>0)return false;const slot=index,spec=playerSkill(p,index),rune=spec.rune;if(p.mana<spec.cost)return false;
  ...
  hitEnemy(e,damage,p,kind='skill',trigger=true){
    ...
    if(kind!=='dot'){this.effect('impact',e.x,e.z,crit?'#fff0bb':CLASSES[p.cls].color,{r:crit?1.4:.75,life:.42,angle:Math.atan2(e.z-p.z,e.x-p.x)});if(crit)this.impact(.026,3);}
  ...
  step(dt,inputs=[]){
    ...
    // Walking outside town never starts a rift; entry is explicit.
    if(this.rift&&['hunting','guardian'].includes(this.rift.phase))this.rift.elapsed+=dt;
    if(this.townChannel){if(this.players.some(p=>p.mark||distance(p,this.townChannel.origins[p.id])>.1)||inputs.some(i=>i&&(i.mx||i.mz||i.attack||i.skills?.some(Boolean)))){this.townChannel=null;this.toast('움직임·공격으로 귀환을 취소했습니다.');}else{this.townChannel.remaining-=dt;if(this.townChannel.remaining<=0){this.townReturn=this.townChannel.origins;this.townChannel=null;this.leaveRift();}}}
    ...
    for(const p of this.players){
      ...
      const at=this.scene==='town'?0:Math.max(1,regionAt(p));if(p.id===0)this.room=at;const region=this.rooms[at];if(at>0&&!region.discovered&&distance(p,region)<region.w){region.discovered=true;if(at===4&&this.rift.phase==='guardian')this.toast('균열에서 수호자를 발견했습니다.','normal',BOSSES[this.bossKey].name);if(at===4&&this.rift.phase==='guardian')this.event('sound',{name:'boss'});}
    ...
    for(const m of this.summons){m.life-=dt;m.cd-=dt;if(m.owner.down||inTown(m.owner))continue;const e=this.closest(m,m.range);if(['wraith','wolf','archer','golem'].includes(m.type)){const target=e||{x:m.owner.x+(m.followX||0),z:m.owner.z+(m.followZ||0)},d=distance(m,target),keep=e?(['wolf','golem'].includes(m.type)?1.1:m.type==='archer'?6:2):.4;if(d>keep)this.move(m,(target.x-m.x)/(d||1)*dt*5.5,(target.z-m.z)/(d||1)*dt*5.5,.25);if(distance(m,m.owner)>20){m.x=m.owner.x;m.z=m.owner.z;}}
    ...
    for(const shot of this.shots){
      ...
      if(!this.walkable(shot.x,shot.z,0)||(this.scene!=='rift'&&inTown(shot))){shot.life=0;continue;}
    ...
    // Rewards come from individual monster rolls and the guardian, not room-clear events.
