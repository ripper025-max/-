...
renderer.frame(1/60);
g.begin(['knight','mage']);g.players.forEach((p,i)=>{p.x=-39+i*2;p.z=-29;});g.spawnEnemy('boss',3,-3,0);g.spawnEnemy('elite',2,1,0);g.spawnEnemy('cultist',-3,-3,0);g.spawnEnemy('brute',-3,0,0);g.spawnEnemy('wisp',1,4,0);
...
for(const cls of Object.keys(CLASSES)){
 g.begin([cls]);const hero=g.players[0];hero.x=-39;hero.z=-29;hero.profile.ranks=[4,4,4,0,0,0];
...
g.begin(['knight']);renderer.camera={x:0,z:0};renderer.frame(.04);
...
for(const cls of Object.keys(CLASSES))for(let source=0;source<5;source++){
 g.begin([cls]);const p=g.players[0];p.x=-26;p.z=-14;p.profile.skillChoices=[source,...[0,1,2,3,4].filter(n=>n!==source).slice(0,2)];renderer.camera={x:p.x,z:p.z};g.skill(p,0,{aim:{x:p.x+2,z:p.z}});for(let i=0;i<24;i++){g.step(.04,[{}]);renderer.frame(.04);}
}
g.begin(['necromancer']);const nec=g.players[0];nec.x=-26;nec.z=-14;['legion','eternal','ossuary'].forEach((key,i)=>nec.profile.equipped[['ring','relic','armor'][i]]=itemForSlot(['ring','relic','armor'][i],nec.cls,3,4,seeded(i),key));g.profileRefresh(nec);g.skill(nec,0,{});renderer.camera={x:nec.x,z:nec.z};renderer.frame(.04);g.save.settings.particles=false;renderer.frame(.04);
